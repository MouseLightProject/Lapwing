function [t,data,chan_name,chan_units,h] = load_abf(file_name,zero_t)

% Loads data from an Axon Binary Format file.
% [t,data,chan_name,chan_units,h] = load_abf(file_name) loads
%   the .abf file  given by file_name.
% The timebase (t) is in s, and is a col vector (n_samples x 1).  
% data is n_samples x n_channels x n_episodes
% chan_name is an n_channels x 1 cell array holding channel names
% chan_units is an n_channels x 1 cell array holding channel units
%   (in strings) 
%
% [t,data,chan_name,chan_units,h] = load_abf(filename,zero_t), if 
%   zero_t is true, shifts the timebase so that it begins at 
%   time zero.
%
...
% read the data
fseek(fid,512*h.lDataSectionPtr,'bof');
% Data representation. 0 = 2-byte integer; 1 = IEEE 4 byte float.
if h.nDataFormat == 0 type_string='short'; else type_string='float'; end
data=zeros([n_times n_channels n_episodes]);
count=0;
for i=1:n_episodes
  [data_temp,count_this]=fread(fid,[n_channels n_times],type_string);
  data(:,:,i)=data_temp';
...
if count ~= n_samples
  fclose(fid); 
  error('Number of samples read not equal to number of samples claimed in header');
end
...
if strcmp(type_string,'short')
    ...
    gain=h.fADCRange/(h.lADCResolution*instrument_scale_factor(chan)*...
                      adc_programmable_gain(chan)*signal_gain(chan));
    offset=signal_offset(chan)+instrument_offset(chan);
    data(:,chan,:)=gain*data(:,chan,:)+offset;  
	end
end
...
i_kept=i_kept_raw(1:n_channels)+1;

% get the channel names, convert to cell array
chan_name_raw=h.sADCChannelName;  % 16x10 char
chan_name=cell(n_channels,1);
for i=1:n_channels
  chan_name{i,1}=deblank(chan_name_raw(i_kept(i),:));
end

% get the channel units, convert to cell array
chan_units_raw=h.sADCUnits;  % 16x8 char 
chan_units=cell(n_channels,1);
for i=1:n_channels
  chan_units{i,1}=deblank(chan_units_raw(i_kept(i),:));
end
